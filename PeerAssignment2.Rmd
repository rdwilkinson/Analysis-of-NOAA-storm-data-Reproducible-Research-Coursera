...
Let's begin with the first, by looking at the total number of injured and fatalities by event type.
```{r}
...
```
Hence, the worst event types would be the following:
```{r}
...
Let's look at this graphically with a bar chart.
```{r}
mergedRank2 <- rbind(highestInjuries, highestFatalities)


ggplot(data = mergedRank2, aes(x = factor(EVTYPE2), y = Num, fill = type)) +
